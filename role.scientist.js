      ...
      if (Object.keys(flagMemory.unBoost).length == 0) {
        if (Object.keys(flagMemory.boosting).length == 0) {
          if (labsFlagMemory && labsFlagMemory.reactionsNeeded) {
            ...
            if (creep.memory.currentReaction !== undefined && creep.memory.currentReaction.length > 0) {
              if (Game.time % 100 == 0) {
        ...
        const lab = Game.getObjectById(flagMemory.unBoost.lab);

        if (creep.store.getUsedCapacity() == 0) {
          if (creep.pos.inRangeTo(lab,2)) {
