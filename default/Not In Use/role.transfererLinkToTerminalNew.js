module.exports = {
  run: function(creep) {
    ...
    const neededNormalTe = 10 * 1000; // 7
    ...
    if (Game.time % 2 == 0) {
      ...
      for (let i = 0;i < possbileResourcesTier0.length; i++) {
        if (resourceInStorage(possbileResourcesTier0[i]) < neededTier0St && resourceInTerminal(possbileResourcesTierNormal[i]) > neededTier0Te) {
          flag.resourcesNeededStorageT0.push(possbileResourcesTier0[i]);
        }
      }
      for (let i = 0;i < possbileResourcesTier1.length; i++) {
        if (resourceInStorage(possbileResourcesTier1[i]) < neededTier1St && resourceInTerminal(possbileResourcesTierNormal[i]) > neededTier1Te) {
          flag.resourcesNeededStorageT1.push(possbileResourcesTier1[i]);
        }
      }
      for (let i = 0;i < possbileResourcesTier2.length; i++) {
        if (resourceInStorage(possbileResourcesTier2[i]) < neededTier2St && resourceInTerminal(possbileResourcesTierNormal[i]) > neededTier2Te) {
          flag.resourcesNeededStorageT2.push(possbileResourcesTier2[i]);
        }
      }
      for (let i = 0;i < possbileResourcesTier3.length; i++) {
        if (resourceInStorage(possbileResourcesTier3[i]) < neededTier3St && resourceInTerminal(possbileResourcesTierNormal[i]) > neededTier3Te) {
    ...
    function goWork(storageType,resourceType) {
      if (storageType == "T") {
        ...
        else if (creep.room.working == true) {
