module.exports = {
  run: function(creep) {
    ...
    if (Game.time % 20 == 0) {
      ...
      for (let i = 0;i < possbileResourcesTierNormal.length; i++) {
        if (resourceInTerminal(possbileResourcesTierNormal[i]) < neededNormalTe && resourceInStorage(possbileResourcesTierNormal[i]) > neededNormalSt) {
          flag.resourcesNeededTerminalTN.push(possbileResourcesTierNormal[i]);
        }
      }
      for (let i = 0;i < possbileResourcesTier0.length; i++) {
        if (resourceInTerminal(possbileResourcesTier0[i]) < neededTier0Te && resourceInStorage(possbileResourcesTier0[i]) > neededTier0St) {
          flag.resourcesNeededTerminalT0.push(possbileResourcesTier0[i]);
        }
      }
      for (let i = 0;i < possbileResourcesTier1.length; i++) {
        if (resourceInTerminal(possbileResourcesTier1[i]) < neededTier1Te && resourceInTerminal(possbileResourcesTier1[i]) > neededTier1St) {
          flag.resourcesNeededTerminalT1.push(possbileResourcesTier1[i]);
        }
      }
      for (let i = 0;i < possbileResourcesTier2.length; i++) {
        if (resourceInTerminal(possbileResourcesTier2[i]) < neededTier2Te && resourceInStorage(possbileResourcesTier2[i]) > neededTier2St) {
        ...
        if (creep.memory.working == false) {
          ...
          if (creep.carry[RESOURCE_ENERGY] > 0 || creep.carry[resourceType] == 0) {
            for (x of RESOURCES_ALL) {
              ...
              if (creep.transfer(creep.room.terminal, x) === 0) {
                break
              }
            }
          }
