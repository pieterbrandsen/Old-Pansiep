module.exports = {
    run: function(creep) {
      let flag2 = Memory.flags[creep.room.name];
      let start = Game.cpu.getUsed();
        ...
        if (creep.memory.working === true) {
          if (target.energy < 250) {
            creep.transfer(target, RESOURCE_ENERGY)
          }
          else if (creep.room.terminal.store.getUsedCapacity(RESOURCE_ENERGY) > 45000 && creep.room.storage.store.getFreeCapacity() > 50000) {
            creep.transfer(creep.room.storage, RESOURCE_ENERGY)
          }
          else if ((creep.room.terminal.store.getUsedCapacity(RESOURCE_ENERGY) < 50000 && creep.room.terminal.store.getFreeCapacity() > 5000) || creep.room.terminal.store.getUsedCapacity(RESOURCE_ENERGY) == null) {
          ...
          else if (target.energy > 400) {
            creep.withdraw(target, RESOURCE_ENERGY)
          }
        }

        // else if (creep.memory.working === false) {
        //   if (flag.workingLinkToTerminal == true) {
        //     if (target.energy > 500) {
        //       if (creep.withdraw(target, RESOURCE_ENERGY) === ERR_NOT_IN_RANGE) {
        //         creep.travelTo(target)
        //       }
        //     }
        //     else {
        //       flag.workingLinkToTerminal = false;
        //     }
        //   }
        //   else {
        //     if (creep.room.terminal.store.getUsedCapacity(RESOURCE_ENERGY) < 50000 && creep.room.storage.store.getUsedCapacity(RESOURCE_ENERGY) > 1000 && target.energy < 500) {
        ...
        flag2.transfererLiTeCpu += Game.cpu.getUsed() - start
    }
};
