module.exports = {
    ...
    run: function(creep) {
        ...
        if (Game.flags["RESOURCE_HYDROGEN"] !== undefined && Game.flags["RESOURCE_HYDROGEN"].room === creep.room || needsResource(RESOURCE_HYDROGEN, 20000)) {
            ...
            if (creep.room.terminal.store.getUsedCapacity(RESOURCE_ENERGY) > 49990 && creep.store.getFreeCapacity(RESOURCE_ENERGY) == 0) {
                if (creep.transfer(creep.room.storage, RESOURCE_ENERGY) === ERR_NOT_IN_RANGE) {
                    creep.travelTo(creep.room.storage)
                }
            }
            else if ((creep.room.terminal.store.getUsedCapacity(RESOURCE_ENERGY) < 49990 && creep.store.getFreeCapacity(RESOURCE_ENERGY) == 0) || creep.room.terminal.store.getUsedCapacity(RESOURCE_ENERGY) == null) {
            ...
            if (creep.room.terminal.store.getUsedCapacity(RESOURCE_ENERGY) < 49990 && creep.room.storage.store.getUsedCapacity(RESOURCE_ENERGY) > 1000 && creep.store.getFreeCapacity(RESOURCE_ENERGY) > 0) {
                if (creep.withdraw(creep.room.storage, RESOURCE_ENERGY) === ERR_NOT_IN_RANGE) {
                    creep.travelTo(creep.room.storage)
                }
            }
            else if (creep.room.terminal.store.getUsedCapacity(RESOURCE_ENERGY) < 49990 && creep.room.storage.store.getUsedCapacity(RESOURCE_ENERGY) < 499900 && creep.store.getFreeCapacity(RESOURCE_ENERGY) > 0) {
                if (creep.withdraw(target, RESOURCE_ENERGY) === ERR_NOT_IN_RANGE) {
                    creep.travelTo(target)
                }
            }
            else if (creep.room.terminal.store.getUsedCapacity(RESOURCE_ENERGY) > 49990 && creep.room.storage.store.getUsedCapacity(RESOURCE_ENERGY) > 1000 && creep.store.getFreeCapacity(RESOURCE_ENERGY) > 0) {
                if (creep.transfer(creep.room.storage, RESOURCE_ENERGY) === ERR_NOT_IN_RANGE) {
                    creep.travelTo(creep.room.storage)
                }
            }
