    ...
    if (freeSpawnIds.length > 0) {
      ...
      function canCreepSpawn(role) {
        ...
        switch(role) {
          ...
          if (flagMemory.labs) {
            // If There Are Less Then One Scientist //
        ...
        if (flagMemory && flagMemory.rolesCount) {
          switch(role) {
            ...
            if (flagMemory.sources.length >= 0) {
              ...
              //if (flagMemory.partsAmount[`${role}-CARRY`] < flagMemory.sources.length * 50)
              if (flagMemory.rolesCount[role] < flagMemory.sources.length)
              result = true;
            }
            break;
            default:
