const withdraw = (creep) => {
  ...
  if (flagMemory.commonMemory.energyStorages.usable <= 500 && creep.memory.targetId === undefined) return 'empty';
    ...
    if (creepMemory.role.includes('upgrade')) {
      ...
      foundStructures.forEach((structure) => {
        ...
        structure.structureType === STRUCTURE_LINK
          ...
          controllerStructure = {type: structure.structureType, id: structure.id};
        }
      });
      ...
      const highestEnergyStructure = flagMemory.commonMemory.energyStructures.sort((a, b) => b.usable - a.usable)[0];
      flagMemory.commonMemory.energyStructures.forEach((structure) => {
        if (structure.id === highestEnergyStructure.id) {
          ...
          flagMemory.commonMemory.energyStorages.usable -= creep.store.getFreeCapacity(RESOURCE_ENERGY);
        }
      });
