const upgradeJob = (creep:Creep): string| undefined=> {
  ...
  const withdrawStructure: StructureContainer|StructureLink|null = Game.getObjectById(flagMemory.commonMemory.controllerStorage.id);
...
const normalJob = (creep:Creep): string|undefined => {
  ...
  if (!creep.memory.targetId) {
    ...
    const highestSourceStructure = flagMemory.commonMemory.energyStructures.filter((s:any) => s.usable <= 2000).sort(
      (a:any, b:any) => b.usable - a.usable,
    )[0];
    ...
    if (creepMemory.role === 'transferer' && flagMemory.commonMemory.spawnEnergyStructures.length > 0) {
      ...
      const structureObj:StructureStorage|StructureTerminal|StructureContainer|StructureLink|null = Game.getObjectById(highestSourceStructure.id);
...
const withdraw = (creep:Creep):string| undefined => {
  ...
  case 'normal':
    ...
    break;
  default:
    if (flagMemory.commonMemory.controllerStorage && creepMemory.role.includes('upgrade') && Game.getObjectById(flagMemory.commonMemory.controllerStorage.id) !== null && flagMemory.commonMemory.controllerStorage.usable > 250) {
