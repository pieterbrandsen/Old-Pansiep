//#region Require('./)
import {Config, MemoryApi_All, CreepRole_Build, CreepRole_Withdraw, CreepRole_Upgrade, CreepRole_Claim, CreepRole_Harvest, CreepRole_Repair, CreepRole_Reserve, CreepRole_Transfer} from "Utils/importer/internals";
//#endregion
...
export class CreepsHelper_Role {
  ...
  private static executeCreep(creep:Creep, job: string): void| string {
    switch (job) {
      case "build":
        return MemoryApi_All.functionRunnerWithCpu(CreepRole_Build.build, Config.creepModuleCpuCost[creep.room.name],job,"+=",creep);
      case "claim":
        return MemoryApi_All.functionRunnerWithCpu(CreepRole_Claim.claim, Config.creepModuleCpuCost[creep.room.name],job,"+=",creep);
      case "harvest":
        return MemoryApi_All.functionRunnerWithCpu(CreepRole_Harvest.source, Config.creepModuleCpuCost[creep.room.name],job,"+=",creep);
      case "mineral":
        return MemoryApi_All.functionRunnerWithCpu(CreepRole_Harvest.mineral, Config.creepModuleCpuCost[creep.room.name],job,"+=",creep);
      case "repair":
        return MemoryApi_All.functionRunnerWithCpu(CreepRole_Repair.repair, Config.creepModuleCpuCost[creep.room.name],job,"+=",creep);
      case "reserve":
        return MemoryApi_All.functionRunnerWithCpu(CreepRole_Reserve.reserve, Config.creepModuleCpuCost[creep.room.name],job,"+=",creep);
      case "transfer":
        return MemoryApi_All.functionRunnerWithCpu(CreepRole_Transfer.transfer, Config.creepModuleCpuCost[creep.room.name],job,"+=",creep);
      case "upgrade":
        return MemoryApi_All.functionRunnerWithCpu(CreepRole_Upgrade.upgrade, Config.creepModuleCpuCost[creep.room.name],job,"+=",creep);
      case "withdraw":
        return MemoryApi_All.functionRunnerWithCpu(CreepRole_Withdraw.withdraw, Config.creepModuleCpuCost[creep.room.name],job,"+=",creep);
      default:
        break;
    }
  }
  ...
  private static isInTargetRoom(creep: Creep, currentRoom: string, targetRoom: string): boolean {
    ...
    else { return this.moveToRoom(creep, targetRoom); }
  };
  ...
  private static pioneer = (creep: Creep): void => {
    ...
    if (!CreepsHelper_Role.isInTargetRoom(creep, creep.room.name, creep.memory.targetRoom)) { return; }
  ...
  private static repairer(creep: Creep) {
    ...
    switch (result) {
      case "full":
        ...
        // Switch to one of the jobs that drains energy
        if (roomMemory.commonMemory.repair.targets.length > 0) { creep.memory.job = "repair"; }
  ...
  private static builder = (creep: Creep) => {
    ...
    switch (result) {
      case "full":
        ...
        // Switch to one of the jobs that drains energy
        if (roomMemory.commonMemory.constructionSites.length > 0) { creep.memory.job = "build"; }
        else { creep.memory.job = "upgrade"; }
        break;
      case "empty":
        // Delete targetId
