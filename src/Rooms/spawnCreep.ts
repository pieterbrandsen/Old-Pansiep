  ...
  const checkIfRoleCanBeSpawned = (role:string, room:Room & MyRoom, memory:CreepMemory) => {
    ...
    case 'transfererLD':
      ...
      if (role === 'transfererLD') {
        ...
        if ((room.controller && room.controller.level >= 6 && room.links.length >= 3) || (room.controller && room.controller.level >= 7 && room.links.length >= 4)) {
    ...
    case 'upgrader':
      ...
      if (room.controller && room.controller.ticksToDowngrade <= 1 * 1000) {
        ...
        if (data.roleCount[role] >= Config.creepsCountMax[shortRoleName] && targetFlagMemory.commonMemory.constructionSites.length === 0) {
          //@ts-ignore
          if (flagMemory.commonMemory.controllerStorage && Game.getObjectById(flagMemory.commonMemory.controllerStorage.id) !== null && Game.getObjectById(flagMemory.commonMemory.controllerStorage.id).store.getFreeCapacity(RESOURCE_ENERGY) > 500) break;
    ...
    case 'builderLD':
      ...
      if (role.includes('LD') && !Game.flags[`builderLD${room.name}`]) break;
        ...
        //@ts-ignore
        if (Memory.flags[`builderLD${room.name}`].room !== undefined) memory.targetRoom = Memory.flags[`builderLD${room.name}`].room
      }
    ...
    case 'reserverLD':
      ...
      if (targetRoom.controller && 
        typeof targetRoom.controller.reservation !== 'object' || (targetRoom.controller && targetRoom.controller.reservation &&
    ...
    case 'mineral':
      ...
      //@ts-ignore
      if (flagMemory.commonMemory.mineral.amount && flagMemory.commonMemory.mineral.amount > 0 && room.controller && room.controller.level >= 6 && room.storage && room.storage.store.getUsedCapacity(flagMemory.commonMemory.mineral.type) < 200*1000) result = true;
      break;
    default:
      break;
    }
