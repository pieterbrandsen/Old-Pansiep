//#region Require('./)
import { Config, GetRandomFreePos, FunctionRunnerWithCpu, IsMemoryPathDefined, RemoteRoomMemory, OwnedRoomStats, TowerHandler, RemoteRoomTimers, RemoteRoomStats } from '../Utils/importer';
//#endregion
...
const remoteRoomHandler = (room:Room & MyRoom):void => {
  ...
  if (!flag || Memory.flags[room.name] === undefined) {
    ...
    FunctionRunnerWithCpu(RemoteRoomTimers, IsMemoryPathDefined(`Memory.stats.rooms.${room.name}.cpu.headModules`), "timers", room);

    // Run all stats collector for remoteRooms //
    FunctionRunnerWithCpu(RemoteRoomStats, IsMemoryPathDefined(`Memory.stats.rooms.${room.name}.cpu.headModules`), "stats", room);
  }
};
//#endregion
