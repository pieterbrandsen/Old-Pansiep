...
export class SpawningHelper {
  public static spawnCreep(room: Room, roomType: string, spawn: StructureSpawn): void {
    ...
    if (!spawn || headSpawn === null) {return;}
    ...
    const initCreepMemory = (room: Room, role: string) => {
      ...
      if (!newMemory.job) { newMemory.job = ''; }
      if (!newMemory.targetId) { newMemory.targetId = ''; }
    ...
    const checkIfRoleCanBeSpawned = (role: string, room: Room, memory: CreepMemory) => {
      ...
      switch (role) {
        ...
        case "pioneer":
          ...
          if (Config.roleCountByRoomByRole[room.name][role] >= Config.creepsCountMax[shortRoleName] * roomMemory.commonMemory.sources.length) {
            break;
          }
        ...
        case "transfererLD":
          ...
          if (role === "transfererLD") {
            if (Config.roleCountByRoomByRole[room.name][role] >= targetRoomMemory.commonMemory.sources.length) { break; }
          } else {
            if (
              (room.controller && room.controller.level >= 6) ||
              (room.controller && room.controller.level >= 7)
            ) {
              if (Config.roleCountByRoomByRole[room.name][role] >= 1) { break; }
            } else {
              if (
        ...
        case "upgrader":
          ...
          if (room.controller && room.controller.ticksToDowngrade <= 1 * 1000) {
            ...
            if (
              ...
              targetRoomMemory.commonMemory.energyStored.capacity / 10 > targetRoomMemory.commonMemory.energyStored.usable
            ) {
              break;
            }
            ...
            if (
              ...
              if (
                ...
                (Game.getObjectById(roomMemory.commonMemory.controllerStorage.id) as StructureContainer).store.getFreeCapacity(RESOURCE_ENERGY) >
                  500
              ) {
                break;
              }
              else if (Config.roleCountByRoomByRole[room.name][role] >= Config.creepsCountMax[shortRoleName]) { break; }
            } else if (
        ...
        case "builderLD":
          // Check if input role is less then max creeps allowed //
          if (Config.roleCountByRoomByRole[room.name][role] >= Config.creepsCountMax[shortRoleName]) { break; }
          ...
          if (targetRoomMemory.commonMemory.constructionSites.length === 0) { break; }
        ...
        case "repairerLD":
          ...
          if (room.controller!.level >= 3) { break; }
  
          // Break if there are no targets left
          if (targetRoomMemory.commonMemory.repair.targets.length === 0) { break; }
        ...
        case "harvesterLD-0":
          ...
          if (targetRoomMemory === undefined) { break; }
  
          if (Game.getObjectById(targetRoomMemory.roomPlanner.room.sources[0].id!) === null) { break; }
        ...
        case "harvesterLD-1":
          ...
          if (targetRoomMemory.commonMemory.sources.length === 1) { break; }
          ...
          if (Game.getObjectById(targetRoomMemory.roomPlanner.room.sources[1].id!) === null) { break; }
    ...
    const getCreepParts = (role: string, room: Room) => {
      ...
      switch (role) {
        ...
        case "harvester-1":
          if (typeof roomMemory.commonMemory.sources[role.split("-")[1] as unknown as number] === "object") {
            const sourceStructureType = roomMemory.roomPlanner.room.sources[role.split("-")[1] as unknown as number].structureType;
    ...
    rolesNeededInRoom.forEach(role => {
      ...
      if (creepBody.body.length === 0) { aCreepHasBeenSpawned[0] = true; }
